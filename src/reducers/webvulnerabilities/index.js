...
import {
  FETCH_WEBVULNERABILITY_BY_ID_SUCCESS,
  FETCH_WEBVULNERABILITY_BY_ID_LOADING,
  FETCH_WEBVULNERABILITY_BY_ID_FAIL,
} from '../../actions/webvulnerability';
...
export default function audits(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case FETCH_WEBVULNERABILITY_BY_ID_LOADING:
    ...
    case FETCH_WEBVULNERABILITY_BY_ID_SUCCESS:
    ...
    case FETCH_WEBVULNERABILITY_BY_ID_FAIL:
      list = _.cloneDeep(state.list);
      index = _.findIndex(list, { id: parseInt(action.result.id, 10) });
